...
uint8_t Vpp_num1 = 4;
uint8_t Vpp_num2 = 4;
uint16_t data_send1[12];
uint16_t data_send2[12];
...
uint8_t TesT_offset(uint32_t * g_adc_dma_data,OSC_ *wave)
{
	...
	{
		 //波形1
		 buffer1[j1] = (((float)(g_adc_dma_data[j1]&0x0000ffff)-32768)*wave->waveform1.Sca_factor[Vpp_num1]+32768)*100/65535;
		 //波形2
		 buffer2[j1] = (((float)(g_adc_dma_data[j1]>>16&0x0000ffff)-32768)*wave->waveform2.Sca_factor[Vpp_num2]+32768)*100/65535;
		j1++;
		zongzhi1 += buffer1[j1];
		zongzhi2 += buffer2[j1];
		 if(j1 >= 320)
...
{
	for(uint16_t i=0;i<160;i++) //处理波形展示
	{
		Show_Lin[i] = ((((float)(adc_dma_data1[i+1]&0x0000ffff)-32768)*3.3+32768)*500/65535);//adc的值放入BUff

		Show_Lin[160+i] = ((((float)(adc_dma_data1[i+1]>>16&0x0000ffff)-32768)*3.3+32768)*500/65535);//adc的值放入BUff
	}
}
...
{
	data_send1[11]=0x5500;
	data_send1[0] = 0x01AA;
	data_send2[11]=0x5500;
	data_send2[0] = 0x02AA;
	for(int i =0;i<16;i++)
	{
		for(int j=0;j<10;j++)
		{
			data_send1[j+1]= Show_Lin[i*10+j];
			data_send2[j+1] = Show_Lin[160+i*10+j];
		}
		if(OSC.waveform1.Vpp>OSC.waveform2.Vpp)
		{
			rt_device_write(serial,0,data_send1,sizeof(data_send1));
		}
		else
		{
			rt_device_write(serial,0,data_send2,sizeof(data_send2));
		}
		rt_thread_mdelay(10);
	}
}
