...
void dac1_SetSinWave(uint16_t _vpp)
{
	...
	for (i = 0; i < 50; i++)
	{
	dac = (double)((sin_rom[i]+114) *_vpp /19800);
	if (dac > 4095)
	{
	dac = 4095;
	}
...
void dac_read(uint8_t data_read[11])
{
	if(data_read[0]==0xea&&data_read[10]==0xfa)
	{
		DAC.DAC_wave_form=data_read[1];
		
		DAC.Freq.Signe=data_read[2];
		DAC.Freq.HCF=data_read[3];
		DAC.Freq.TCF=data_read[4];
		DAC.Freq.UCF=data_read[5];
		
		DAC.Vpp.TV = data_read[6];
		DAC.Vpp.UV = data_read[7];
		DAC.Vpp.HmV = data_read[8];
		DAC.Vpp.TmV = data_read[9];
	}
